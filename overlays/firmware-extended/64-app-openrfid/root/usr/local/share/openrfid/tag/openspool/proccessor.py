...
class OpenspoolTagProcessor(NdefTagProcessor):
    ...
    def __openspool_parse_payload(self, payload : bytes) -> GenericFilament | None:
        ...
        try:
            ...
            if  brand == "Spoolman":
                try:
                    spool_id = data.get('spool_id', 'FAILURE')
                    if spool_id == 'FAILURE':
                        raise ValueError("Missing 'spool_id' for Spoolman filament")

                    response = requests.get(f"http://dueroserver.tail5c1e21.ts.net:7912/api/v1/spool/{spool_id}", timeout=3)
                    self.logger.debug(f"Spoolman API response status: {response.status_code}")

                except Exception as e:
                    self.logger.debug(f"Spoolman loading data failed: {str(e)}")
